...
class EPQ(nn.Module):
    ...
    def xstrip(self): #removes the 0 vectors from the PQ table
        print("simplified PQ...")
...
#        self.n_nodes, self.dimension = self.x.size()
        ...
        if self.pre in ['SIGN', 'H2GC']:
            num_sets = x.size(1) // self.dimension
            padded_dimension = x.size(1) // num_sets
            x_list = []
            for i in range(num_sets):
                x_list.append(x[:, i*padded_dimension:(i*padded_dimension)+self.dimension])
