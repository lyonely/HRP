...
def result_container(qnt_param, output_data_dict, *args, **kwargs):
    x = sizeTracker(qnt_param, *args, **kwargs)
    y = PrettyTable(['best epoch','type', 'acc', 'sens', 'spec', 'prec', 'f1'])
    y.add_row([output_data_dict['best_epoch'], 'val', f"{output_data_dict['max_val_acc']:.4f}", f"{output_data_dict['max_val_sens']:.4f}", f"{output_data_dict['max_val_spec']:.4f}", f"{output_data_dict['max_val_prec']:.4f}", f"{output_data_dict['max_val_f1']:.4f}"])
    y.add_row([output_data_dict['best_epoch'], 'test', f"{output_data_dict['max_test_acc']:.4f}", f"{output_data_dict['max_test_sens']:.4f}", f"{output_data_dict['max_test_spec']:.4f}", f"{output_data_dict['max_test_prec']:.4f}", f"{output_data_dict['max_test_f1']:.4f}"])
    print(y)
    return x,y
...
def calculate_model_size(model):
    ...
    for buffer in model.buffers():
        buffer_size = buffer.numel() * buffer.element_size()  # Size in bytes
        total_buffer_size += buffer_size
        
    total_size = total_param_size + total_buffer_size  # Total size in bytes
    total_size_mb = total_size / (1024 ** 2)  # Convert bytes to megabytes
    
    print(f"Model parameters size: {total_param_size / (1024 ** 2):.2f} MB")
    print(f"Model buffers size: {total_buffer_size / (1024 ** 2):.2f} MB")
...
def modelSize(qnt_param, **kwargs):
    ...
    if model == 'GCN':
        ...
        layer1_wt = kwargs['n_feature'] * kwargs['n_class'] * 4
        model_size_b = layer1_wt
        if qnt_param[0].wt_qparam.sqnt:
            layer1_wt /= (32/qnt_param[0].wt_qparam.bits[0])
        model_size_a = layer1_wt
